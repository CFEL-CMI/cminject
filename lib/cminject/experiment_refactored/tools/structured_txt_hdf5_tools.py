...
def txt_to_hdf5(infile_name: str, outfile_name: str) -> None:
    ...
    for i in range(len(headers)):
        ...
        # Z iterates the slowest (see the docstring), so we reorder to get x,y,z order like typical products
        # (e.g. itertools.product) would generate. hdf5_to_dataframe will construct the index in x,y,z product order,
        # where X iterates the slowest, so we reshape the data according to this.
