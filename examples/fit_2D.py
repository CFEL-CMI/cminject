  ...
  if x is not None and y is not None:
    ...
    for i in range(len(ID)):
      ...
      if (x[int(ID[i])]>X[0] and x[int(ID[i])]<X[1]
            and y[int(ID[i])]>Y[0] and y[int(ID[i])]<Y[1]
              and vx[int(ID[i])]>VX[0] and vx[int(ID[i])]<VX[1]
                and vy[int(ID[i])]>VY[0] and vy[int(ID[i])]<VY[1]
                  and vz[int(ID[i])]>VZ[0] and vz[int(ID[i])]<VZ[1]):
        mask[i] = True
    return mask
